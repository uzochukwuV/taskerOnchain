...
/**
 * @title TakeProfitAdapter
 * @notice Automatically sells tokens when price reaches target
 * @dev Works with Uniswap V2-compatible DEXes
 */
contract TakeProfitAdapter is Ownable {
    ...
    /**
     * @notice Execute take profit order
     ...
     * @return success Whether take profit executed
     */
    function execute(
        uint256 _taskId,
        address _protocol,
        ...
        (
            ...
            uint256 takeProfitPrice,  // Target price to trigger sell
            address recipient,
            address creator
        ...
        if (currentPrice < takeProfitPrice) {
            return false; // Price not reached target yet
        }
        ...
        uint256 minAmountOut = (amountToSell * takeProfitPrice * (10000 - maxSlippageBps)) / 10000 / 1e18;
    ...
    /**
     * @notice Check if take profit should trigger
     * @param _router DEX router
     * @param _tokenIn Token to sell
     * @param _tokenOut Token to receive
     * @param _takeProfitPrice Target price
     * @return bool Whether take profit should trigger
     */
    function shouldTrigger(
        ...
        try this._getCurrentPrice(_tokenIn, _tokenOut, _router) returns (uint256 currentPrice) {
            return currentPrice >= _takeProfitPrice;
        } catch {
            return false;
        }
    }
    ...
    /**
     * @notice Preview take profit execution
     * @param _router DEX router
     * @param _tokenIn Token to sell
     * @param _tokenOut Token to receive
